...
all_tables = ["clients", "derivative_products", "market_data", "transactions"]
...
def createdatabaseengine(schema_: str) -> Engine:
    '''Creates  database engine for MYSQL database for the provided schema in the parameter. \
    ...
    connection_string = aws_rds_database_connection
    #engine = create_engine(connection_string, echo=True,connect_args={'options': '-csearch_path={}'.format(schema_)})
...
def getbedrockllm():
    ...
    return llm_claude
'''
def getgpt4llm(temperature=0):
    
    llm = ChatOpenAI(model_name= os.getenv('MODEL_NAME'), temperature=temperature)
    return llm
